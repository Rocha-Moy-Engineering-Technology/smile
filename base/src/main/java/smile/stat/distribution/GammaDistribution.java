...
/**
 * The Gamma distribution is a continuous probability distributions with a scale
 ...
 * random variables, each of which has a rate parameter of &theta;). The gamma
 * distribution is frequently a probability model for waiting times; for
 * instance, the waiting time until death in life testing. The probability
 * density function is f(x; k,&theta;) = x<sup>k-1</sup>e<sup>-x/&theta;</sup> /
 * (&theta;<sup>k</sup>&Gamma;(k)) for {@code x > 0} and k, &theta; {@code > 0}.
 * <ul>
 ...
 * <li>If k is an integer, the gamma distribution is an Erlang distribution and
 * is the probability distribution of the waiting time until the k-th "arrival"
 * in a one-dimensional Poisson process with intensity 1/&theta;.
 * <li>If X<sup>2</sup> &sim; &Gamma;(3/2, 2a<sup>2</sup>), then X has a
 * Maxwell-Boltzmann distribution with parameter a.
 * </ul>
 * In Bayesian inference, the gamma distribution is the conjugate prior to many
 * likelihood distributions: the Poisson, exponential, normal (with known mean),
 * Pareto, gamma with known shape, and inverse gamma with known shape parameter.
 *
 * @author Haifeng Li
 */
public class GammaDistribution extends AbstractDistribution implements ExponentialFamily {
	...
	// throw new IllegalArgumentException("Gamma random number generator support
	// only integer shape parameter.");
	// }
	...
	// theta + " " + k + " " + thetaGammaK);
	// System.out.println(
	// Math.pow(x / theta, k - 1) + " " + Math.exp(-x / theta) + " " + Math.exp(-x /
	// theta) / thetaGammaK
	// );
	// System.out.println();
	// }
