...
/**
 * Linear model. In linear regression, the model specification is that the
 * dependent variable is a linear combination of the parameters (but need not be
 * linear in the independent variables). The residual is the difference between
 * the value of the dependent variable predicted by the model, and the true
 ...
 * estimated parameters. Commonly used checks of goodness of fit include the
 * R-squared, analysis of the pattern of residuals and hypothesis testing.
 ...
 * assumptions. Although examination of the residuals can be used to invalidate
 * a model, the results of a t-test or F-test are sometimes more difficult to
 ...
 * parameters will not follow normal distributions and complicate inference.
 * With relatively large samples, however, a central limit theorem can be
...
public class LinearModel implements DataFrameRegression {
	 ...
	 * regression line approximates the real data points. An R<sup>2</sup> of 1.0
	 * indicates that the regression line perfectly fits the data.
	 * <p>
	 * In the case of ordinary least-squares regression, R<sup>2</sup> increases as
	 * we increase the number of variables in the model (R<sup>2</sup> will not
	 * decrease). This illustrates a drawback to one possible use of R<sup>2</sup>,
	 * where one might try to include more variables in the model until "there is no
	 * more improvement". This leads to the alternative approach of looking at the
	 * adjusted R<sup>2</sup>.
	 */
	double RSquared;
	/**
	 * Adjusted R<sup>2</sup>. The adjusted R<sup>2</sup> has almost same
	 * explanation as R<sup>2</sup> but it penalizes the statistic as extra
	...
	double pvalue;
	/**
	 * First initialized to the matrix (X<sup>T</sup>X)<sup>-1</sup>, it is updated
	 * with each new learning instance.
	 */
	Matrix V;
	...
	public LinearModel(Formula formula, StructType schema, Matrix X, double[] y, double[] w, double b) {
		...
		int df_model = this.p + (this.bias ? 0 : (this.b == 0 ? 0 : 1));
		int n = X.nrow();
		int df_residuals = n - df_model;
		df = df_residuals;
		...
		if (Double.isInfinite(F)) {
			...
			if (df_residuals > 0 && F > 0.0) {
				if (df_model > 0) {
					// Compute the p-value using the F-distribution
					FDistribution fDist = new FDistribution(df_model, df_residuals);
				...
				// Handle edge cases where F is non-positive or df_residuals is zero
				pvalue = Double.NaN;
			}
		}
	}
	...
	/**
	 * Returns the t-test of the coefficients (including intercept). The first
	 * column is the coefficients, the second column is the standard error of
	 * coefficients, the third column is the t-score of the hypothesis test if the
	 * coefficient is zero, the fourth column is the p-values of test. The last row
	 * is of intercept.
	 *
	...
	/**
	 * Returns R<sup>2</sup> statistic. In regression, the R<sup>2</sup> coefficient
	 * of determination is a statistical measure of how well the regression line
	 * approximates the real data points. An R<sup>2</sup> of 1.0 indicates that the
	...
	/**
	 * Returns adjusted R<sup>2</sup> statistic. The adjusted R<sup>2</sup> has
	 * almost same explanation as R<sup>2</sup> but it penalizes the statistic as
	...
	/**
	 * Growing window recursive least squares with lambda = 1. RLS updates an
	 * ordinary least squares with samples that arrive sequentially.
	 *
	...
	/**
	 * Recursive least squares. RLS updates an ordinary least squares with samples
	 * that arrive sequentially.
	 * <p>
	 * In some adaptive configurations it can be useful not to give equal importance
	 * to all the historical data but to assign higher weights to the most recent
	 * data (and then to forget the oldest one). This may happen when the phenomenon
	 * underlying the data is non stationary or when we want to approximate a
	 * nonlinear dependence by using a linear model which is local in time. Both
	 * these situations are common in adaptive control problems.
	 *
	 * @param x      training instance.
	 * @param y      response variable.
	 * @param lambda The forgetting factor in (0, 1]. The smaller lambda is, the
	 *               smaller is the contribution of previous samples to the
	 *               covariance matrix. This makes the filter more sensitive to
	 *               recent samples, which means more fluctuations in the filter
	 *               coefficients. The lambda = 1 case is referred to as the growing
	 *               window RLS algorithm. In practice, lambda is usually chosen
	 *               between 0.98 and 1.
	 */
	...
	public String toString() {
		...
		builder.append(String.format("%10.4f  %10.4f  %10.4f  %10.4f  %10.4f%n", MathEx.min(r), MathEx.q1(r),
				MathEx.median(r), MathEx.q3(r), MathEx.max(r)));
		...
		if (ttest != null) {
			...
			for (int i = 0; i < p; i++) {
				builder.append(String.format("%-15s %10.4f %10.4f %10.4f %10.4f %s%n", predictors[i], ttest[i][0],
						ttest[i][1], ttest[i][2], ttest[i][3], Hypothesis.significance(ttest[i][3])));
			}
		...
		builder.append(
				String.format("Multiple R-squared: %.4f,    Adjusted R-squared: %.4f%n", RSquared, adjustedRSquared));
