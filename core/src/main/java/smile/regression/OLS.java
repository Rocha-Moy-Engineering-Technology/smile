...
/**
 * Ordinary least squares. In linear regression, the model specification is that
 * the dependent variable is a linear combination of the parameters (but need
 * not be linear in the independent variables). The residual is the difference
 * between the value of the dependent variable predicted by the model, and the
 * true value of the dependent variable. Ordinary least squares obtains
 * parameter estimates that minimize the sum of squared residuals, SSE (also
 * denoted RSS).
 * <p>
 * The OLS estimator is consistent when the independent variables are exogenous
 * and there is no multicollinearity, and optimal in the class of linear
 * unbiased estimators when the errors are homoscedastic and serially
 * uncorrelated. Under these conditions, the method of OLS provides
 ...
 * can be cast in order to make the OLS technique applicable. Each of these
 * settings produces the same formulas and same results, the only difference is
 * the interpretation and the assumptions which have to be imposed in order for
 * the method to give meaningful results. The choice of the applicable framework
 * depends mostly on the nature of data at hand, and on the inference task which
 * has to be performed.
 * <p>
...
public class OLS {
	...
	/**
	 * Fits an ordinary least squares model. The hyper-parameters in
	 * <code>prop</code> include
	 * <ul>
	 * <li><code>smile.ols.method</code> (default "svd") is a string (svd or qr) for
	 * the fitting method
	 * <li><code>smile.ols.standard.error</code> (default true) is a boolean. If
	 * true, compute the estimated standard errors of the estimate of parameters
	 ...
	 * @param stderr    if true, compute the standard errors of the estimate of
	 *                  parameters.
	...
	public static LinearModel fit(Formula formula, DataFrame data, String method, boolean stderr, boolean recursive) {
		...
		if (n <= p) {
			throw new IllegalArgumentException(
					String.format("The input matrix is not over determined: %d rows, %d columns", n, p));
		}
